...
Swallow repository into distinct checkout `/home/rioyokota/projects/swallow`; read its
own instructions and task ledger completely; verify branch, worktree, recent
...
**Decision D-001 — selected:** use exact checkout
`/home/rioyokota/projects/swallow` and retain the unchanged Harness-owned
runtime boundary.
...
`/home/rioyokota/projects/students`, and `swallow` at
`/home/rioyokota/projects/swallow`. The recommended implementation makes the
tracked monitor, helper, resilient launcher, global launch sentinels, and
cold-start/continuation contracts target-aware. Every recovered thread first
reconstructs Harness control state, then reads its mapped repository's own
instructions and declared durable ledger completely, inspects that
repository's Git and mutable external state, and resumes only its first
recorded unverified action. Focused tests must prove the mapping and reject
unknown targets.
...
**Next action:** obtain one changed-input owner decision. Recommended: preserve
the symlinked owner workspace and receipt files unchanged, replace D-001/D-002
with fresh direct, non-symlinked canonical clones at
`/mnt/nfs-03/safe/Users/rioyokota/codex-workspaces/students` and
`/mnt/nfs-03/safe/Users/rioyokota/codex-workspaces/swallow`, then resume the
frozen sequence from repository preparation. Do not clone Swallow or modify
either project before that decision. Do not restart a project session before
its repository and the target-aware control plane are protected and active.
...
PR #426 passed `portable-phase1` at exact r3 head
`991ba770ad6435147709eef35fb063a0e0d5f56a` and merge-merged as
`2caa515e7ad7762878ceeb0f516f0f03682865b4`. r3 ran once and again
terminally refused before any watcher or operation phase. Its 655-byte
mode-0600 journal proves only helper stop/resume and `verified=0`; no
app-server connection or lifecycle request occurred. All three pre-request
receipts remain immutable and all runtime health returned to the accepted
state.

The r3 fresh-idle proof occurred immediately before `SIGSTOP`, but its base
child query was still a separate observation. r4 makes this proof atomic:
after fresh idle it signals the exact helper, waits for Linux state `T`, checks
the stopped parent has zero children, and exposes that frozen proof exactly
once to the unchanged base gate. A child causes immediate helper resume and
refusal.

Private r4 wrapper
`/run/user/5035/harness-t347-mirror-convergence/controller-r4.py` is current
user, mode `0700`, one link, 6,423 bytes, SHA-256
`bc610c2c3a40f6b3c22d2ba1447fe18f9a80ac572890b0f0d136c5bdae116aa7`.
...
PR #432 passed `portable-phase1` at exact v4 head
`fdcf4804fb3ca2ca2a36f14b2430ee02145d1073` and merge-merged as
`5bcaff6a7390393a07075f801ed9c11cda3982b0`. v4 ran once, stopped and
resumed all three watchers, opened one metadata-only app-server connection,
and terminally refused before any operation. It remains `verified=0` with no
lifecycle request.

The corrected value-free app read shows canonical Harness/Students/Swallow
statuses `active/active/idle`, all three exact app names already matching, and
all three extras `notLoaded` with exact rollout paths. The two database
name-match booleans remain false, so the frozen `thread/name/set` writes are
still required for persistent mirror parity; their app readback is already
idempotently exact.

Private v5 wrapper
`/run/user/5035/harness-t347-mirror-convergence/controller-v5.py` is current
user, mode `0700`, one link, 4,407 bytes, SHA-256
`124dee9f4dceb35e6ae174889c38677762bba2b64afebf0baf39772e2764d231`.
It retains the exact transport loader, requires all canonical app names exact
