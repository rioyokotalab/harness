...
- Validate in order: exact-owner discovery, one owner per repair, then
  ...
  below 10% of task runtime by narrowing or splitting slow owners. Tests use
  deterministic handshakes, never elapsed-time sleeps, polling, or reruns.
...
- Add only nonredundant focused behavior tests and use `harness validate`;
  follow the discovery/repair/final order above before protected publication.
