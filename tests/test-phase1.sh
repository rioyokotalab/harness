...
[ "$(cat "$remote_codex_log")" = \
    "-A -t ab exec bash -lic 'cd \"\$HOME\" && exec harness codex-resilient --run --name harness --last'" ] ||
...
[ "$(cat "$remote_codex_log")" = \
    "-A -t invented exec bash -lic 'cd \"\$HOME\" && exec harness codex-resilient --run --name harness --last'" ] ||
