...
| `abq` | RHEL 9.4 | Bash, x86_64 | modules, PBS Pro, Singularity CE | installed |
...
PTY SSH connection, the normal interactive login hook, and a direct resilient
Harness Codex launch while preserving native `codex` resolution. It accepts
any non-local logical host backed by a strict regular file
